...
io.sockets.on('connection', function (socket) { // First connection
	...
	socket.on('message', function (data,opponentsocket_id) { // Broadcast the message to all
		if(pseudoSet(socket))
		{
			var transmit = {date : new Date().toISOString(), pseudo : returnPseudo(socket),sendsocketuser:socket.id,recieversocketuser:opponentsocket_id, message : data};
			socket.broadcast.emit('message', transmit);
			
		}
	});
	socket.on('setPseudo', function (data,udid) { // Assign a name to the user
		...
		if (pseudoArray.indexOf(data) == -1) // Test if the name is already taken
		{
			...
			socket.set('pseudo', name, function(){
				...
				socket.emit('pseudoStatus','ok', socket.id);
	...
	socket.on('findAllUsers', function (data) { // Assign a name to the user
		
		socket.emit('findAllUsersResponse', socketchannels);
	});
	socket.on('gameRequset', function (from,to,coinfee) { // Assign a name to the user
		...
		if (index==-1) {
				...
				if(pseudoSet(socket))
				{
			      ...
			      ssocket.emit('gameRequstinvitaion', fromname,from,to,toname,coinfee);
		}else{
			
			 socket.emit('playingResponseBusy', from,to);
		}
		
		
	});
	socket.on('agrrePlay', function (socket_id,from,opponenttype,letter_Bag) { // Assign a name to the user
		...
		if (opponenttype=="real") {
			...
			scoreChanel[schanel][socket_id]={"score":0,letterbag:letter_Bag};
			scoreChanel[schanel][from]={"score":0,letterbag:letter_Bag};
			...
			scoreChanel[schanel][from].letterbag=newbag;
			    ...
			    socket.emit('playingResponse', "agree",socket_id,from,playid,schanel,fiveletterword,opponenttype,letter7array,letter7array1,scoreChanel[schanel][socket_id].letterbag);
			     ssocket.emit('playingResponse', "agree",socket_id,from,playid,schanel,fiveletterword,opponenttype,letter7array,letter7array1,scoreChanel[schanel][socket_id].letterbag);
			    
			
			 setTimeout(function(){
				 io.sockets.emit('playingOver', scoreChanel[schanel],socket_id,from,"Game Over");
			 ...
			 socket.emit('playingResponseBusy', socket_id,from);
		}
			//code
			...
			scoreChanel[schanel][from].letterbag=newbag;
			 ...
			 setTimeout(function(){
				socket.emit('playingOver', scoreChanel[schanel],socket_id,from,"Game Over");
				 var index=currentgameplayers.indexOf(socket_id);
	...
	socket.on('discardPlay', function (socket_id,from) { // Assign a name to the user
		
		socket.emit('playingResponse', "discard",socket_id,from);
	});
	socket.on('swaprack', function (socket_id,opponentsocket_id,schanel,reqdecoy,reqdecoyusedlet) { // Assign a name to the user
			       ...
			       if (currentletbag.length > 0) {
				      ...
				      if (currentletbag.length >=p.length) {
					       ...
					       for (l = 0; l < p.length; l = l + 1) {
						       
									       var letter=currentletbag[p[l]];
				      ...
				      socket.emit('swaprackResponse',reqdecoy,reqdecoyletters);
	...
	socket.on('submitPlay', function (socket_id,opponentsocket_id,current_playername,opponentusername,current_playing_id,connectedwords,schanel,opponenttype,reqdecoy,letterused) { // Assign a name to the user
		console.log("submit play"+socket_id+"--"+opponentsocket_id);
		...
		if(opponenttype=="real"){
			...
			if (index!=-1) {
			    ...
			    var timestamp = Math.round(+new Date()/1000);
			       ...
			       socket.emit('submitPlayResponse', socket_id,opponentsocket_id,current_playername,current_playing_id,connectedwords,playid,scoreChanel[schanel],reqdecoy,reqdecoyletters,scoreChanel[schanel][socket_id].letterbag);
		               ssocket.emit('submitPlayResponse', socket_id,opponentsocket_id,current_playername,current_playing_id,connectedwords,playid,scoreChanel[schanel],reqdecoy,reqdecoyletters,scoreChanel[schanel][socket_id].letterbag);
			...
			if (index!=-1) {
			    ...
			    var timestamp = Math.round(+new Date()/1000);
			     ...
			     console.log("current plaing id is not found for the played word "+connectedwords.playword);
			    
			     socket.emit('replayAi', socket_id,reqdecoy,letterused);
			}
		}
	...
	socket.on('placeTile', function (socket_id,opponentsocket_id,current_playername,opponentusername,current_playing_id,allWords,schanel,opponenttype) { // Assign a name to the user
	   var ssocket = clients[opponentsocket_id];
	   if(opponenttype=="real"){
		 ssocket.emit('opponentPlaceTile', socket_id,opponentsocket_id,current_playername,opponentusername,current_playing_id,allWords,schanel);
	
	   }else if (opponenttype=="virtual") {
		
		socket.emit('opponentPlaceTile', socket_id,opponentsocket_id,current_playername,opponentusername,current_playing_id,allWords,schanel);
	
	   }
        });
	socket.on('endGame', function (player1id,player2id,schanel) { // Assign a name to the user
		
		io.sockets.emit('playingOver', scoreChanel[schanel],player1id,player2id,"Game Over"); 
        });
	
	socket.on('virtualWordWrong', function (id,opponentid,schanel,reqdecoyusedlet) { // Assign a name to the user
		...
		if (currentletbag.length > 0) {
			for(var j=0;j < reqdecoyusedlet.length;j++){
		      ...
		      socket.emit('replayVirtual', id,reqdecoyletters); 
		}else{
